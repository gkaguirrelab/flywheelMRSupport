...
%
%  'projectName'          - The name of the project on Flywheel in which
%                           these data are stored. A limit of the current
%                           code is that data cannot be combined across
%                           projects.
...
%  'includeFreeSurferLicenseFile' - Char vector of the form "true" or
%                           "false". Determines if the gear requires
%                           specification of a freeSurfer license file.
...
%   "nifit", "bval", "bvec".
%
%   Row 5 is the first of four rows with Boolean settings. This row sets if
...
%
%   Row 7 indicates if the inout is an Analysis file. Boolean.
%
%   Row 8 inidicates if the file name is to be an exact match to a file on
...
%
%   	subjectID/sessionLabel/acquisitionLabel/acquisitionfileName     or
%       subjectID/sessionLabel/gearName/analysisfileName                or
%       subjectID/sessionLabel/sessionFileName
%
% Inputs:
    ...
    for jj=nParamCols+1:nInputCols
        ...
        if logical(str2double(char(paramsTable{IsSessionFileRow,jj}))) || ...
            ...
            if logical(str2double(char(paramsTable{IsAcquisitionFileRow,jj})))
                % If a file entry was specified, go find that.
                if length(entry)==4
